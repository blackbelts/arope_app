...
@Component({
  selector: 'app-non-motor-claims-survey',
  templateUrl: './non-motor-claims-survey.page.html',
  styleUrls: ['./non-motor-claims-survey.page.scss'],
})
export class NonMotorClaimsSurveyPage implements OnInit {
  ...
  constructor(public odooApi: OdooApiService,private router: Router, public shared: SharedService, public navCtrl: NavController) { 
    this.odooApi.callOdooMethod('arope.broker', 'get_non_motor_claim_survey',
  ...
  goToDetails(item){
    ...
    this.router.navigate(['non-motor-claim-survey-details'], navigationExtras);
  }
  ionViewWillEnter() {
    this.odooApi.callOdooMethod('arope.broker', 'get_non_motor_claim_survey',
    {data: this.shared.userId}).then(res => {
